...
;( function( window ) {
	...
	function shuffleMArray( marray ) {
		var arr = [], marrlen = marray.length;
		if( null == marray[ 0 ] ) return null; //moy
		var inArrLen = marray[0].length;
			...
			moveItems = function() {
				...
				self.allItems.forEach( function( item, i ) {
					...
					if(self.items.indexOf(item) === self.current && self.started && iter === 0) {
						...
						//nueva class para que permita editar el contenido de atrás
						classie.addClass( self.currentItem, 'backface-vis' );
					}
					else {
	...
	Photostack.prototype._getSizes = function() {
		...
		this.centerItem = { x : Math.ceil( this.sizes.inner.width / 2 - this.sizes.item.width / 2 ), y : Math.ceil( this.sizes.inner.height / 2 - this.sizes.item.height / 2 ) };
	}
